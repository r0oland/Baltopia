...
////////////////////////////////////////////////////////////////////////////////
void loop() {
  int32_t realRes = measure_resistance();
  NanoSensorData.soilRes = realRes/10; // better use available range
...
int32_t measure_resistance(){
  ...
  for (int i=0; i < N_READS; i++) {
    ...
    resistance += int32_t(float(COMP_RESISTOR)*(supplyReadOut-sensorReadOut)/sensorReadOut+0.5);
  ...
  if (resistance < 0)
    resistance = 0; // use this to check for infinite resistance

  return resistance;
  // convert to 0.1k Ohm to better cover range we have
}
